...
class Search {
    ...
    <h2 class="search-overlay__section-title">General information</h2>
        ...
        ${posts.map(item => `<li><a href="${item.link}">${item.title.rendered}</a> ${item.type == 'post' ? `by ${item.authorName}` : ''}</li>`).join('')}
        ${posts.length ? '</ul>' : ''}
        </ul>
      `);
      this.isSpinnerVisible = false;
    ...
    openOverlay(){
        ...
        console.log("our open method just Fired!");
        this.isOverlayOpen = true;   
    }
    closeOverlay(){
        this.searchOverlay.removeClass("search-overlay--active");
        $("body").removeClass("body-no-scroll");
        console.log("our close method just Fired!");
        this.isOverlayOpen = false;
    }
